    ...
    */
    public func items<
        ...
        return { source in
            ...
            return source.subscribeProxyDataSource(ofObject: self.base, dataSource: dataSource, retainDataSource: true) { [weak mainCollectionView = self.base] (_: RxCollectionViewDataSourceProxy, event) -> Void in
                guard let mainCollectionView = mainCollectionView else {
                    return
                }
                dataSource.collectionView(mainCollectionView, observedEvent: event)
            }
        }
    }
}
