...
CREATE TABLE IF NOT EXISTS `baseball`.`game` (
                                                 ...
                                                 `pitcher_player_id` INT NOT NULL,
                                                 `batter_player_id` INT NOT NULL,
                                                 `base1_player_id` INT NULL,
                                                 `base2_player_id` INT NULL,
                                                 `base3_player_id` INT NULL,
                                                 PRIMARY KEY (`id`),
                                                 INDEX `fk_game_team1_idx` (`home_team_id` ASC),
                                                 INDEX `fk_game_team2_idx` (`away_team_id` ASC),
                                                 INDEX `fk_game_player1_idx` (`pitcher_player_id` ASC),
                                                 INDEX `fk_game_player2_idx` (`batter_player_id` ASC),
                                                 INDEX `fk_game_player3_idx` (`base1_player_id` ASC),
                                                 INDEX `fk_game_player4_idx` (`base2_player_id` ASC),
                                                 INDEX `fk_game_player5_idx` (`base3_player_id` ASC),
                                                 ...
                                                 CONSTRAINT `fk_game_player1`
                                                     FOREIGN KEY (`pitcher_player_id`)
                                                         REFERENCES `baseball`.`player` (`id`)
                                                         ON DELETE NO ACTION
                                                         ON UPDATE NO ACTION,
                                                 CONSTRAINT `fk_game_player2`
                                                     FOREIGN KEY (`batter_player_id`)
                                                         REFERENCES `baseball`.`player` (`id`)
                                                         ON DELETE NO ACTION
                                                         ON UPDATE NO ACTION,
                                                 CONSTRAINT `fk_game_player3`
                                                     FOREIGN KEY (`base1_player_id`)
                                                         REFERENCES `baseball`.`player` (`id`)
                                                         ON DELETE NO ACTION
                                                         ON UPDATE NO ACTION,
                                                 CONSTRAINT `fk_game_player4`
                                                     FOREIGN KEY (`base2_player_id`)
                                                         REFERENCES `baseball`.`player` (`id`)
                                                         ON DELETE NO ACTION
                                                         ON UPDATE NO ACTION,
                                                 CONSTRAINT `fk_game_player5`
                                                     FOREIGN KEY (`base3_player_id`)
                                                         REFERENCES `baseball`.`player` (`id`)
                                                         ON DELETE NO ACTION
                                                         ON UPDATE NO ACTION)
    ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `baseball`.`inning` (
                                                   ...
                                                   `score` INT NOT NULL,
                                                   `inning_name` VARCHAR(10) NOT NULL,
                                                   PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `baseball`.`pitch_history` (
                                                          ...
                                                          INDEX `fk_history_player1_idx` (`pitcher_id` ASC),
                                                          INDEX `fk_history_player2_idx` (`batter_id` ASC),
...
CREATE TABLE IF NOT EXISTS `baseball`.`batting_history` (
                                                            ...
                                                            INDEX `fk_batting_history_player1_idx` (`batter_id` ASC),
                                                            CONSTRAINT `fk_batting_history_game1`
                                                                FOREIGN KEY (`game_id`)
                                                                    REFERENCES `baseball`.`game` (`id`)
                                                                    ON DELETE NO ACTION
                                                                    ON UPDATE NO ACTION,
                                                            CONSTRAINT `fk_batting_history_player1`
                                                                FOREIGN KEY (`batter_id`)
                                                                    REFERENCES `baseball`.`player` (`id`)
                                                                    ON DELETE NO ACTION
                                                                    ON UPDATE NO ACTION)
