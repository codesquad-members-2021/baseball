...
public class SQL {
    ...
    public static String FIND_PLAYER_BY_TEAM_NAME = "SELECT p.id as pid, p.type, p.name, r.id as rid, r.outs, r.hits, r.at_bat, r.average FROM player p inner JOIN record r ON r.player = p.id WHERE p.team = :teamName order by p.id";
    public static String FIND_PLAYER_BY_PLAYER_ID = "select p.id as pid, p.name, p.type, r.id as rid, r.outs, r.hits, r.at_bat, r.average from player p inner JOIN record r ON r.player = p.id where p.id= :id";
    public static String UPDATE_PLAYER_RECORD = "update record set at_bat = :atBat, hits = :hits, outs = :outs, average = :average where id = :id";
